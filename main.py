...
class PlaneWar:
    ...
    def _check_collision_myplane_bulletsupply(self):
        ...
        # 检测子弹补给的分组中是否有子弹补给与我方飞机发生了碰撞
        list_collided = pygame.sprite.spritecollide(self.my_plane,
                                                    self.bullet_supply_group,
                                                    False,
                                                    pygame.sprite.collide_mask)

        # 如果检测到有子弹补给与我方飞机发生了碰撞
        if len(list_collided) > 0:
            # 遍历所有发生碰撞的子弹补给：
    ...
    def _check_collision_myplane_bombsupply(self):
        ...
        # 检测炸弹补给的分组中是否有炸弹补给与我方飞机发生了碰撞
        list_collided = pygame.sprite.spritecollide(self.my_plane,
                                                    self.bomb_supply_group,
                                                    False,
                                                    pygame.sprite.collide_mask)

        # 如果检测到有炸弹补给与我方飞机发生了碰撞
        if len(list_collided) > 0:
            # 遍历所有发生碰撞的炸弹补给：
            for bomb_supply in list_collided:
                ...
                bomb_supply.kill()
                # 如果炸弹的数量小于3
                if self.visual_bomb_group.bomb_number < constants.BOMB_INITIAL_NUMBER:
                    # 炸弹的数量加1
                    self.visual_bomb_group.bomb_number += 1
    ...
    def _update_current_score(self, enemy):
        ...
        if type(enemy) == SmallEnemy:
            # 当前得分加上摧毁小型敌机后的得分
            self.score_board.current_score += \
                constants.DESTROY_SMALL_ENEMY_SCORE
        # 如果摧毁的敌机是中型敌机
        elif type(enemy) == MidEnemy:
            # 当前得分加上摧毁中型敌机后的得分
            self.score_board.current_score += \
                constants.DESTROY_MID_ENEMY_SCORE
        # 如果摧毁的敌机是大型敌机
        elif type(enemy) == BigEnemy:
            # 当前得分加上摧毁大型敌机后的得分
            self.score_board.current_score += \
                constants.DESTROY_BIG_ENEMY_SCORE
    ...
    def _update_enemies_supplies_speed(self):
        ...
        elif self.score_board.current_level == 3 and \
                not self.is_update_level3_speed:
            # 更新小型敌机的速度
